...
# 所有resluts都會儲存到以下資料夾
OUTPUT_DIR = "./FullFT_DebertaLarge/"
os.makedirs(OUTPUT_DIR, exist_ok=True)


# 指定GPU
os.environ["CUDA_VISIBLE_DEVICES"] = "7"  # 若有多卡，可改成 "6,7"
...
for epoch in range(1, EPOCHS+1):
    ...
    for batch in loop:
        ...
        outputs = model(input_ids=input_ids, attention_mask=attention_mask, labels=labs)
        loss = outputs.loss
    ...
    line = f"Epoch {epoch}, train_loss={avg_loss:.4f}, val_acc={val_acc:.4f}"
    print(line)
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "fullft_model_best.pt"))
        print(f"[INFO] Saved best model with val_acc={val_acc:.4f}")

# save model
torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "fullft_model_last.pt"))
...
print(f"[INFO] Overall test acc = {test_acc:.4f}")
...
test_line = f"Test => Overall:{test_acc:.4f}, SST-2:{sst2_acc:.4f}, MNLI:{mnli_acc:.4f}, CLINC:{clinc_acc:.4f}"
with open(os.path.join(OUTPUT_DIR, "training_log.txt"), "a", encoding="utf-8") as f:
    f.write(test_line+"\n")

log_memory("After Testing")
print("[INFO] Done, results in:", OUTPUT_DIR)
